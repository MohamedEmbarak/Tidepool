...
const INITIAL: DiveState = { depth: 0, nearby: [], opened: [], hovered: null, complete: false };
...
export default function Expedition() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        engineRef.current = createDive(canvasRef.current, {
    ...
    void start();
    return () => { alive = false; engineRef.current?.dispose(); engineRef.current = null; if (messageTimer.current) clearTimeout(messageTimer.current); };
  }, [announce, attempt]);
  useEffect(() => {
  ...
  const toggleSound = async () => {
    if (sound) { audio.disable(); setSound(false); }
      ...
      catch { announce('Sound could not start. You can still explore silently.'); }
    }
  };
  ...
  return <main className="expedition" data-ready={ready} data-depth={dive.depth}>
    <canvas ref={canvasRef} className="ocean-canvas" tabIndex={0} aria-label="Tidepool underwater world. Scroll or swipe to dive. Arrow keys change depth. Tab to explore nearby objects." />
    ...
    <header className="expedition-header">
      ...
      <div className="expedition-tools">
        <button className="sound-button" onClick={() => void toggleSound()} aria-label={sound ? 'Mute sound' : 'Enable sound'} aria-pressed={sound}><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 9v6h4l5 4V5L7 9H3z"/>{sound ? <path d="M16 8a6 6 0 010 8m3-11a10 10 0 010 14"/> : <path d="m17 9 5 6m0-6-5 6"/>}</svg></button>
    ...
    <footer className="dive-console">
      ...
      <div className="depth-buttons"><button onClick={() => jump(dive.depth - 9)} aria-label="Ascend" disabled={dive.depth < 0.1}>↑</button><button onClick={() => jump(dive.depth + 9)} aria-label="Descend" disabled={dive.depth > 119.9}>↓</button></div>
    </footer>
