...
export default function Expedition() {
  ...
  return <main className="expedition" data-ready={ready} data-depth={dive.depth} data-zoom={dive.zoom} data-angle={dive.angle} data-fish-count={dive.fishCount} data-seals={dive.seals} data-arch-hits={dive.archHits} data-opened={dive.opened.join(',')}>
    ...
    <div className="habitat-caption" key={zoneIndex}><p className="eyebrow"><span className="status-dot"/>{String(zoneIndex + 1).padStart(2, '0')} / {zone.name}</p><h1>{complete && zoneIndex === 3 ? 'The ocean remembers.' : zone.title}</h1><p className="habitat-description">{complete && zoneIndex === 3 ? 'A collection of memories. One very old friend. Stay a while.' : dive.depth > 108 ? dive.seals ? `${dive.seals} rune seals bind the moon. Each discovery breaks one.` : 'The seals have fallen. Touch the moon.' : zone.description}</p></div>
    ...
    <footer className="dive-console">
      ...
      <details className="explore-help"><summary>How to explore <span aria-hidden="true">?</span></summary><div><p className="eyebrow">Follow your curiosity</p><dl><dt>Swipe up / down</dt><dd>Dive through the water</dd><dt>Drag sideways</dt><dd>Orbit the full 3D habitat</dd><dt>Tap an object</dt><dd>Open it or add it to your journal</dd><dt>Hold the golden fish</dt><dd>It spits out a hidden treasure</dd><dt>Tap the sealed arch</dt><dd>Three touches break it open</dd><dt>Collect a discovery</dt><dd>Shatter one of the moon’s rune seals</dd><dt>Touch fish / plants / rocks</dt><dd>Scatter, sway, and stir the sand</dd><dt>Drag a creature</dt><dd>Play with the current</dd><dt>Pinch / two fingers</dt><dd>Zoom in / pan around</dd><dt>Double-tap open water</dt><dd>Centre the view</dd></dl><p>Keyboard: up/down to dive, left/right to orbit, Tab to discover, 0 to centre.</p></div></details>
    </footer>
