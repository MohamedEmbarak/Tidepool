...
import { approach, discoveryLocations, MAX_DEPTH, randomSequence, RELICS, type RelicId } from './catalog';
import { clearMaterials, jelly, makeRelics, shellModel } from './models';
...
export async function createDive(canvas: HTMLCanvasElement, options: Options, signal?: AbortSignal): Promise<DiveEngine> {
  ...
  let ratio = renderPixelRatio(canvas.clientWidth, canvas.clientHeight, devicePixelRatio, memory); renderer.setPixelRatio(ratio);
  ...
  try {
    ...
    const revealTime = new Map<RelicId, number>();
    ...
    const archive = breakableArch(), ruin = habitats.register(archive.root, 'structure', 'The sealed arch', true);
    const chest = treasureChest(), chestProp = habitats.register(chest.root, 'structure', 'An old treasure chest', true);
    const carrier = placeModel(assets.fish3, 2.8), carrierProp = habitats.register(carrier.root, 'structure', 'Hold the golden fish', true);
    const carrierMaterials = new Map<T.Material, T.Material>();
    const warm = (m: T.Material) => {
      if (!carrierMaterials.has(m)) { const copy = m.clone(); if (copy instanceof T.MeshStandardMaterial && copy.color.r + copy.color.g + copy.color.b > 0.2) copy.color.lerp(new T.Color('#f2b454'), 0.68); carrierMaterials.set(m, copy); }
      return carrierMaterials.get(m)!;
    };
    carrier.root.traverse(o => { if (o instanceof T.Mesh) o.material = Array.isArray(o.material) ? o.material.map(warm) : warm(o.material); });
    const mouth = new T.Vector3(), releaseFrom = new T.Vector3();
    ...
    archive.root.traverse(o => { if (o instanceof T.Mesh) o.userData.relic = 'rune'; });
    chest.root.traverse(o => { if (o instanceof T.Mesh) o.userData.relic = 'medallion'; });
    carrier.root.traverse(o => { if (o instanceof T.Mesh) o.userData.carrier = true; });
    ...
    function placeDiscoveries() {
      ...
      const at = (id: RelicId) => locations.find(r => r.id === id)!;
      const pearl = at('pearl'), compass = at('compass'), key = at('key'), rune = at('rune'), moon = at('moon'), amber = at('amber'), medallion = at('medallion');
      moveProp(shellProp, pearl.x, -pearl.depth - 0.01, pearl.z);
      moveProp(shipProp, compass.x - 1, -compass.depth - 2.3, compass.z - 2.8);
      moveProp(keyProp, key.x, -key.depth - 1.25, key.z + 0.15);
      moveProp(ruin, rune.x, -rune.depth, rune.z);
      moveProp(chestProp, medallion.x, -medallion.depth + 0.32, medallion.z);
      moveProp(carrierProp, amber.x - 2.2, -amber.depth, amber.z);
      seals.root.position.set(moon.x, -moon.depth, moon.z);
    }
    placeDiscoveries();
    ...
    function activate(id: RelicId, action: DiscoveryAction = 'accessible') {
      ...
      const result = locks.interact(id, action);
      if (result === 'none') return;
      if (result === 'hold-needed') { options.onHint('Hold the golden fish gently until it spits out its treasure.'); return; }
      if (result === 'sealed') { options.onHint(`${seals.remaining()} rune seals remain. Each other discovery breaks one.`); return; }
      if (result === 'cracked') { effects.burst(ruin.home, false, true, options.reduced); options.onHint(`The arch fractures. ${3 - locks.archHits} more ${locks.archHits === 2 ? 'touch' : 'touches'}.`); reportAt = -1; return; }
      if (result === 'opened') {
        revealTime.set(id, time);
        if (id === 'amber') { carrier.root.localToWorld(mouth.set(1.35, 0, 0)); releaseFrom.copy(mouth); }
        const hints: Partial<Record<RelicId, string>> = { pearl: 'A little sunrise. Touch the pearl to keep it.', key: 'The garden opens. Touch the key to take it.', amber: 'A little hiccup! The fish returns its amber. Touch it to keep it.', medallion: 'The lid opens. A medallion waits inside.', rune: 'The seal breaks. Take the tide sigil from the falling stones.' };
        options.onHint(hints[id]!); effects.burst(id === 'amber' ? releaseFrom : model.group.position, false, id === 'rune', options.reduced); reportAt = -1; return;
      }
      const sinceReveal = time - (revealTime.get(id) ?? -100);
      if (sinceReveal < (options.reduced ? 0.25 : id === 'amber' ? 1.35 : 0.55)) return;
    ...
    function hitAt(x: number, y: number): Pick | null {
      setRay(x, y);
      pickCandidates.length = 0; pickHits.length = 0;
      for (const object of targets) if (visible(object.userData.owner ?? object)) pickCandidates.push(object);
      ray.intersectObjects(pickCandidates, false, pickHits);
      for (const hit of pickHits) {
        ...
        if (data.carrier) return { kind: 'creature', id: !opened.has('amber') && !found.has('amber') ? 'amber' : undefined, object: carrier.root, point: hit.point };
    ...
    function react(picked: Pick | null, click = false) {
      if (picked?.object === carrier.root) carrierProp.pulse = click ? 1 : 0.4;
    ...
    const asGesture = (picked: Pick | null): GestureTarget | null => picked ? { kind: picked.kind === 'relic' ? 'relic' : picked.kind === 'fish' || picked.kind === 'creature' ? 'creature' : 'scenery', holdable: picked.id === 'amber' && !opened.has('amber'), value: picked } : null;
    const gestures = new DiveGestures({
      hit: p => { const picked = hitAt(p.x, p.y); heldCarrier = picked?.object === carrier.root; waterPoint(p.x, p.y, 1.2, picked); react(picked); return asGesture(picked); },
      start: () => { diveVelocity = 0; dragged = null; heldCarrier = false; hovered = null; hoverId = null; },
      ...
      tap: (target, p) => { const picked = target?.value as Pick | undefined; const end = hitAt(p.x, p.y); waterPoint(p.x, p.y, 3, end); if (picked?.id && end?.id === picked.id) activate(picked.id, 'tap'); else react(end, true); },
      hold: target => { const picked = target.value as Pick, relic = RELICS.find(r => r.id === picked.id); if (picked.id === 'amber' && !opened.has('amber')) activate('amber', 'hold'); else if (relic) options.onHint(`${relic.name} · ${relic.clue}`); navigator.vibrate?.(12); },
      zoom: ratio => { rig.zoom = T.MathUtils.clamp(rig.zoom * ratio, 0.8, 1.8); resize(); reportAt = -1; },
      release: velocity => { dragged = null; heldCarrier = false; diveVelocity = options.reduced ? 0 : T.MathUtils.clamp(-velocity * rig.height / height, -40, 40); },
      reset: resetView,
    });
    ...
    const onMove = (e: PointerEvent) => {
      ...
      hovered = hoverId ? (hoverId === 'pearl' && !opened.has('pearl') ? 'Open the shell' : hoverId === 'key' && !opened.has('key') ? 'Part the fronds' : hoverId === 'amber' && !opened.has('amber') ? 'Hold the golden fish · a treasure inside' : hoverId === 'rune' && !opened.has('rune') ? `Break the arch seal · ${3 - locks.archHits} touches` : hoverId === 'medallion' && !opened.has('medallion') ? 'Open the old chest' : hoverId === 'moon' && seals.remaining() ? `${seals.remaining()} rune seals · find the other memories` : RELICS.find(r => r.id === hoverId)!.name) : picked?.scenery?.name ?? (picked ? 'Make a little current' : null);
    ...
    function frame(now: number) {
      ...
      if (!paused) {
        ...
        shell.lid.rotation.x += ((opened.has('pearl') || found.has('pearl') ? -1.18 : 0) - shell.lid.rotation.x) * Math.min(1, dt * (options.reduced ? 40 : 5));
        ...
        chest.update(opened.has('medallion') || found.has('medallion'), dt, options.reduced);
        archive.update(opened.has('rune') || found.has('rune'), locks.archHits, dt, options.reduced);
        if (carrier.root.visible) {
          carrierProp.pulse *= Math.exp(-dt * 3);
          if (!options.reduced) carrier.mixer.update(dt);
          const spit = time - (revealTime.get('amber') ?? -100), retch = spit < 1.35 ? Math.sin(spit * 15) * (1 - spit / 1.35) : 0;
          if (dragged !== carrier.root && !heldCarrier) { temp.copy(carrierProp.home); if (!options.reduced) { temp.y += Math.sin(time * 1.2) * 0.16; temp.z += Math.sin(time * 0.6) * 0.18; } carrier.root.position.lerp(temp, Math.min(1, dt * 4)); }
          carrier.root.scale.set(1, 1 + (options.reduced ? 0 : retch * 0.18) + carrierProp.pulse * 0.06, 1); carrier.root.rotation.z = options.reduced ? 0 : retch * 0.12 + Math.sin(time * 5) * carrierProp.pulse * 0.06;
        }
        relics.forEach(({ relic, group, model }, i) => {
          ...
          if (at !== undefined) { const elapsed = time - at; group.visible = elapsed < 0.8; group.scale.setScalar(Math.max(0, 1 - elapsed * 1.6)); group.position.y = -locations[i].depth + elapsed * 1.8; group.rotation.y = elapsed * 5; }
            ...
            group.visible = !found.has(relic.id) && (!['amber', 'rune', 'medallion'].includes(relic.id) || opened.has(relic.id));
            if (relic.id === 'amber' && opened.has('amber') && !found.has('amber')) {
              const elapsed = time - (revealTime.get('amber') ?? -100), t = options.reduced ? 1 : T.MathUtils.clamp((elapsed - 0.3) / 1.05, 0, 1);
              group.visible = elapsed >= (options.reduced ? 0 : 0.3); temp.set(locations[i].x, -locations[i].depth, locations[i].z);
              group.position.copy(releaseFrom).lerp(temp, t); group.position.y += Math.sin(t * Math.PI) * 0.55; group.scale.setScalar(0.3 + t * 0.7);
            }
          }
        ...
        if (time - reportAt > 0.1) {
          reportAt = time;
          const state: DiveState = { depth: Math.round(current * 10) / 10, zoom: Math.round(rig.zoom * 100) / 100, angle: Math.round(rig.yaw * 180 / Math.PI), fishCount: SCHOOL_FISH_COUNT + 4, seals: seals.remaining(), archHits: locks.archHits, nearby: RELICS.filter(r => !found.has(r.id) && Math.abs(r.depth - current) < rig.height * 0.37).map(r => r.id), opened: [...opened], hovered, complete };
    ...
    return { goTo, resetView, activate, setPaused(value) { paused = value; visibility(); }, reset(seed) {
      ...
      placeDiscoveries(); shell.lid.rotation.x = 0; keyCover.children.forEach((leaf, i) => { leaf.rotation.z = (i - 1) * 0.08; }); current = 0; goTo(0); resetView(); rig.update(0, 0, true); reportAt = -1;
    }, dispose() {
      ...
      carrier.mixer.stopAllAction(); carrier.mixer.uncacheRoot(carrier.mixer.getRoot()); pickCandidates.length = 0; pickHits.length = 0;
      disposeObject(scene); Object.values(assets).forEach(a => disposeObject(a.scene)); environment?.dispose(); clearMaterials(); renderer.dispose(); renderer.forceContextLoss();
    } };
  } catch (error) {
    disposeObject(scene); Object.values(assets).forEach(a => disposeObject(a.scene)); environment?.dispose(); clearMaterials(); renderer.dispose(); renderer.forceContextLoss(); throw error;
  }
}
